...
int alg; /*算法，1为EDF，2为RMS*/
int curr_proc = -1;
int demo_time = 100; /*演示时间*/

task* tasks;
pthread_mutex_t proc_wait[100]; /*任务数不会超过100*/
...
int main(int argc, char** argv) {
	...
	pthread_mutex_lock(&main_wait); /*下次执行此语句就会将所在线程等待*/
	pthread_mutex_init(&idle_wait, NULL);
	pthread_mutex_lock(&idle_wait); /*下次执行此语句就会将所在线程等待*/
	...
	pthread_create(&idle_proc, NULL, idle, NULL); /*创建闲逛进程,但还没运行几行肯定就返回到主线程*/
	for (int i = 0; i < task_num; ++i) { /*创建实时进程,但还没运行几行肯定就返回到主线程*/
	...
	for (int i = 0; i < demo_time; ++i) {
		if ((curr_proc = select_proc(alg)) != -1) {
			...
			pthread_mutex_unlock(&idle_wait); //pthread_mutex_lock与sem_wait的区别？？？
			pthread_mutex_lock(&main_wait);
		}
